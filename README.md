...
positional arguments:
  {regen,purge}
    ...
    purge               remove all rpm2swidtag plugin locally-generated SWID
                        tags

```

When the `rpm2swidtag` DNS plugin is enabled in
`/etc/dnf/plugins/rpm2swidtag.conf`, any DNF operation which adds, removes,
